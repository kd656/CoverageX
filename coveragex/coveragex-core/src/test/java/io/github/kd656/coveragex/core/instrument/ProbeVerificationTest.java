...
import io.github.kd656.coveragex.core.fixtures.conditions.*;
import io.github.kd656.coveragex.core.fixtures.loops.*;
import io.github.kd656.coveragex.core.fixtures.switches.*;
import io.github.kd656.coveragex.core.fixtures.exceptions.*;
import io.github.kd656.coveragex.core.fixtures.lambdas.*;
import io.github.kd656.coveragex.core.fixtures.methods.*;
import io.github.kd656.coveragex.core.instrument.InstrumentationVerifier;
...
class ProbeVerificationTest {
    ...
    void shortCircuitAnd_nullInput_firstBranchShortCircuits() throws Exception {
        ...
        List<io.github.kd656.coveragex.api.data.ProbeMetadata> meta = v.probeMetadata();
        boolean[] hits = v.probeHits();
        int pairsHit = (int) meta.stream()
            .filter(pm -> pm instanceof io.github.kd656.coveragex.api.data.ProbeMetadata.BranchProbe bp
                && hits[bp.probeId()])
            .count();
    ...
    void shortCircuitOr_null_firstConditionShortCircuits() throws Exception {
        ...
        boolean[] hits = v.probeHits();
        int hitCount = 0;
        for (io.github.kd656.coveragex.api.data.ProbeMetadata pm : v.probeMetadata()) {
            if (pm instanceof io.github.kd656.coveragex.api.data.ProbeMetadata.BranchProbe bp && hits[bp.probeId()]) {
                hitCount++;
            }
        }
    ...
    void nestedTernary_positiveValue_firstTernaryTrueSecondNotEvaluated() throws Exception {
        ...
        long hitBranches = meta.stream()
            .filter(pm -> pm instanceof io.github.kd656.coveragex.api.data.ProbeMetadata.BranchProbe bp && hits[bp.probeId()])
            .count();
    ...
    void lambdaWithCondition_branchProbeRegisteredInsideSyntheticMethod() throws Exception {
        ...
        boolean lambdaBranchFound = v.probeMetadata().stream()
            .anyMatch(pm -> pm instanceof io.github.kd656.coveragex.api.data.ProbeMetadata.BranchProbe bp
    ...
    void chainedFourOperands_allTrue_allFourPairsEvaluated() throws Exception {
        ...
        long trueHit = v.probeMetadata().stream()
            .filter(pm -> pm instanceof io.github.kd656.coveragex.api.data.ProbeMetadata.BranchProbe bp
                && bp.direction() == io.github.kd656.coveragex.api.data.ProbeMetadata.BranchDirection.TRUE
                && hits[bp.probeId()])
            .count();
